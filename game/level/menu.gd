...
func intro_dialogue() -> void:
	if not DialogueManager.hide_dialogue:
		...
		#var box2 = await DialogueManager.say("(Hover over the text boxes to keep them open)", DialogueManager.NPC.UNKNOWN)
		#await Tutorial.wait_for_action()
		#DialogueManager.remove_static_box(box2)
		var box3 = await DialogueManager.say("You signed up for this, tough it out", DialogueManager.NPC.UNKNOWN)
		await Tutorial.wait_for_action()
